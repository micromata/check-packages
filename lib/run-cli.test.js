...
describe('run(cli)', () => {
  beforeEach(() => {
    ...
    readWhitelistFile.mockReturnValue([]);
  ...
  it('shows help when called without any params', () => {
    const cli = {
      input: [],
      flags: {},
      showHelp: cliShowHelp
  ...
  describe('when analysis crashed', () => {
    ...
    it('calls spinner.fail', () => {
      ...
      expect(spinnerFail).toHaveBeenCalledWith('Dependency analysis error: Reading whitelist failed! Error: file not found');
    });
  ...
  describe('when analysis failed', () => {
    beforeEach(() => {
      readWhitelistFile.mockReturnValue([ 'react' ]);
      readDependencies.mockReturnValue({
        tree: {
          name: 'angular',
          version: '5.0.0'
